/**
 * Amazon's root CAs for RDS in us-east-2, used to verify the database's
 * certificate (see `createClient` in db.ts). Inlined as a string rather than
 * shipped as a .pem file because the deploy zips `dist/index.js` alone —
 * infra/09-lambda.sh and the deploy-lambda CI job both do — so anything the
 * function reads at runtime has to be inside the esbuild bundle.
 *
 * This is the regional bundle, matching AWS_REGION in infra/env.sh. Moving the
 * database to another region means fetching that region's bundle, or the
 * global one, otherwise connections fail with a self-signed-certificate error.
 * Refresh with:
 *
 *   curl -sSL https://truststore.pki.rds.amazonaws.com/us-east-2/us-east-2-bundle.pem
 *
 * Three roots (RSA2048 G1, RSA4096 G1, ECC384 G1); the RSA2048 one expires
 * 2061, so no rotation is due before then unless AWS reissues.
 */
export const RDS_CA_BUNDLE = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

Pqj2
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`;
