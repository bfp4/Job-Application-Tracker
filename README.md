...
- **Autofill from a job link** — paste an AshbyHQ, Greenhouse, Workday, Workable or Work at a Startup (YC) posting URL and the title, company, locations, salary and full description are filled in from the board's own public data, ready to review before saving.
...
- **Autofill reads JSON, not markup.** Every provider in [`server/src/services/scrapers/`](server/src/services/scrapers) consumes the same structured payload the board's own careers page renders from — no DOM traversal, no headless browser, nothing that breaks on a redesign. Four of them are plain API clients: Ashby and Greenhouse document theirs, while Workday's `/wday/cxs/{tenant}/{site}/job/{path}` and Workable's `/api/v1/accounts/{account}/jobs/{shortcode}` are undocumented but unauthenticated and stable. Work at a Startup is the one that costs a page fetch: it's an Inertia.js app, so it serializes the page's entire prop tree into a single `data-page` attribute and the client hydrates from that. Reading it is one GET and one JSON parse. (Inertia will hand back that JSON directly, but only to a request echoing the current per-deploy asset hash — which is itself only in the HTML, so there is nothing to save.) Adding a board is one `Scraper` appended to the registry — the route and the client contract don't change.
- **What the boards don't give back.** The normalized shape is the union of what the five expose, so some fields are structurally empty rather than missing by accident. Workday has no compensation field at all, so `salary` is always null there; Work at a Startup publishes no date on a posting, so `postedDate` is always null there. Neither Workday nor Workable returns a company display name on the posting: Workable has a separate account endpoint, fetched alongside the job purely for the name (and allowed to fail — it degrades to the slug rather than sinking the autofill, which is why `labella-associates` comes back as "LaBella Associates" and not "Labella Associates"); Workday has nothing equivalent, so its name is title-cased from the tenant slug. In the other direction, Work at a Startup gives back *more* than the shape has room for, and the extra is worth keeping: equity is appended to the free-text salary (`$150K - $250K + 0.10% - 1.00% equity`), and the skills list and interview process are appended to the description under labels, since both are things you'd otherwise re-read the posting for. Every autofilled field lands in an editable form behind a *review before saving* note, which is what makes best-effort acceptable here.
- **Autofill is not auto-apply.** It fills in *this tracker's* form from a public posting. It does not fill in the employer's application form: Workday, Workable and Work at a Startup all gate that behind an authenticated candidate session (a YC account, in the last case), and Workday's is a stateful multi-step wizard whose questionnaire differs per requisition. Driving those would mean automating a logged-in browser as the user — a different feature with a different risk profile, and out of scope for a server-side API client.
