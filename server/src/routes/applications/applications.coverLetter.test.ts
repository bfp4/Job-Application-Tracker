...
vi.mock("../../services/agents/coverLetter/coverLetter", async (importOriginal) => ({
  ...(await importOriginal<typeof import("../../services/agents/coverLetter/coverLetter")>()),
...
describe("cover-letter endpoints", () => {
  ...
  it("POST does not refund when the save fails after a billed generation", async () => {
    ...
    prismaMock.coverLetter.upsert.mockRejectedValueOnce(new Error("pool timeout"));
