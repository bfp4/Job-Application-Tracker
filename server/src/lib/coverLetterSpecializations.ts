...
/**
 * Per-field guidance injected into the cover-letter prompt, keyed by the user's
 * Career Specialization (the same setting the tailored resume, resume tips,
 * LinkedIn notes, and application answers follow). Labels live in
 * careerSpecializations.ts.
 *
 ...
 */
export const COVER_LETTER_GUIDANCE: Record<CareerSpecialization, string> = {
  GENERAL:
...
/** The cover-letter guidance for a field, falling back to General. */
export function coverLetterGuidance(
  ...
  return {
    ...
    guidance: COVER_LETTER_GUIDANCE[key],
  };
}
